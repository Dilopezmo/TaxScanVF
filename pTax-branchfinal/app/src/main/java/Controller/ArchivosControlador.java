...
public class ArchivosControlador {
    ...
    public void subirFactura(Uri image_url, ArrayList<String> campos) {
        Uri file = image_url;
        DatabaseReference facturasRef = database.getReference("facturas");
        String uid = FirebaseAuth.getInstance().getCurrentUser().getUid();
        DatabaseReference facturaRef = facturasRef.child(uid).child(file.getLastPathSegment().substring(0, file.getLastPathSegment().length() - 4));
        facturaRef.setValue(campos).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                // Éxito
                Log.d("Firebase", "Factura subida exitosamente");
            } else {
                // Error
                Log.e("Firebase", "Error al subir la factura", task.getException());
            }
        });
